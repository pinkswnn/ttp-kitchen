...
function attachFocusMode() {
  ...
  const focusables = inquiryForm.querySelectorAll("input, textarea, select, button");
  focusables.forEach((el) => {
    el.addEventListener("focus", () => {
      enableFocusMode();
    });
    el.addEventListener("blur", () => {
      // If focus moved outside the form, exit focus mode
      window.setTimeout(() => {
        const active = document.activeElement;
        if (!active || !inquiryForm.contains(active)) disableFocusMode();
      }, 0);
    });
  });
