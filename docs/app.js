...
/* Entrance animations (IO + optional stagger) */
(function(){
  ...
  document.querySelectorAll('[data-stagger]').forEach((container)=>{
    const step = parseInt(container.getAttribute('data-stagger') || '90', 10);
    const kids = Array.from(container.querySelectorAll('.anim'));
    kids.forEach((el,i)=>{
      el.style.transitionDelay = `${Math.min(i*step, 900)}ms`;
